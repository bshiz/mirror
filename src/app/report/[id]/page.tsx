...
export default function ReportPage() {
  ...
  if (loading) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', fontFamily: "Georgia, serif", fontSize: 22, fontStyle: 'italic', color: '#7a7770' }}>Loading report...</div>
  if (!meeting?.report) return <div style={{ padding: 40, fontFamily: 'system-ui, sans-serif', color: '#7a7770' }}>Report not found.</div>
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '44px 40px 80px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10, gap: 24 }}>
        <div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 14, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#7a7770', background: '#e8e8e8', padding: '3px 10px', borderRadius: 20 }}>
              {meeting.meeting_type || 'Meeting'}
            </span>
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 14, color: '#7a7770' }}>
        ...
        <div style={{ background: '#e8e8e8', borderRadius: 10, padding: '18px 24px', textAlign: 'center', flexShrink: 0 }}>
          <div style={{ fontFamily: "Georgia, serif", fontSize: 40, fontStyle: 'italic', color: scoreColor(r.overall_score), lineHeight: 1, marginBottom: 2 }}>
            {r.overall_score}
          </div>
      ...
      {/* Inferred goal */}
      <p style={{ fontFamily: "Georgia, serif", fontStyle: 'italic', fontSize: 16, color: '#7a7770', marginBottom: 28, paddingBottom: 28, borderBottom: '1px solid #e0e0e0' }}>
        <strong style={{ fontStyle: 'normal', fontWeight: 400, color: '#3a3834' }}>Inferred goal:</strong> {meeting.inferred_goal}
      </p>

      {/* Summary */}
      <div style={{ background: '#eef1f7', borderLeft: '3px solid #4a6fa5', padding: '16px 20px', marginBottom: 32, fontSize: 16, lineHeight: 1.7, color: '#3a3834', fontStyle: 'italic', fontFamily: "Georgia, serif" }}>
        {r.summary}
      </div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 14, color: '#7a7770', marginTop: 1 }}>{p.tagline}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#e0e0e0', transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▾</div>
                </div>
              </div>
              {isOpen && (
                <div style={{ padding: '0 20px 20px', borderTop: '1px solid #e8e8e8' }}>
                  <p style={{ fontFamily: "Georgia, serif", fontStyle: 'italic', fontSize: 16, color: '#3a3834', padding: '14px 0 12px', lineHeight: 1.6, margin: 0 }}>{p.verdict}</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {p.points.map((pt, j) => (
                      <div key={j} style={{ display: 'flex', gap: 10, fontSize: 16, lineHeight: 1.65, color: '#3a3834' }}>
                        <span style={{
                          ...
                          color: pt.type === 'strength' ? '#1a6b5a' : pt.type === 'gap' ? '#4a6fa5' : '#b8860b'
                        }}>{pt.type}</span>
                        <div>
                          <div>{pt.text}</div>
                          {pt.quote && (
                            <div style={{ background: '#f2f2f2', borderLeft: '2px solid #e0e0e0', padding: '8px 12px', marginTop: 6, fontFamily: "'DM Mono', monospace", fontSize: 14, color: '#7a7770', lineHeight: 1.6 }}>
                              "{pt.quote}"
                            </div>
                          )}
                        </div>
      ...
      {/* Actions */}
      <div style={{ background: '#0f0e0c', color: '#f2f2f2', borderRadius: 10, padding: 28, marginBottom: 32 }}>
        <h3 style={{ fontFamily: "Georgia, serif", fontSize: 20, fontStyle: 'italic', marginBottom: 18, opacity: 0.9 }}>
          Before your next meeting.
        </h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {r.actions.map((a, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', fontSize: 16, lineHeight: 1.6, opacity: 0.85 }}>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 14, color: '#4a6fa5', minWidth: 18, marginTop: 3 }}>0{i + 1}</span>
              <span>{a}</span>
            </div>
          ))}
        </div>
